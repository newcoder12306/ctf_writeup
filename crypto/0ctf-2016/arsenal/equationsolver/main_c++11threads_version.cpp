...
unsigned char values[4][256];
LUTTable abcdtables[4];
atomic_int answercount(0);
mutex stdoutmutex;

void bruteforce(int k180start, int k180end)
{
    const int numofelements=5;
    for (int k180=k180start;k180<k180end;k180++)
    {
        #define EXPERIMENTAL
        #ifdef EXPERIMENTAL
        if (answercount > 0)
        ...
        for (int k171=0;k171<256;k171++)
        {
            ...
            for (int k162=0;k162<256;k162++)
            {
                ...
                for (int k193=0;k193<256;k193++)
                {
                    ...
                    {
                        const auto ret = fastinvmixcolumns({(uint8_t)(mix[0][i]^mix[0][i+1]), (uint8_t)(mix[1][i]^mix[1][i+1]), (uint8_t)(mix[2][i]^mix[2][i+1]), (uint8_t)(mix[3][i]^mix[3][i+1])});
                        ...
                        for (auto bit = brange.first; bit != brange.second; bit++)
                        {
                            ...
                            for (auto cit = crange.first; cit != crange.second; cit++)
                            {
                                ...
                                for (auto dit = drange.first; dit != drange.second; dit++)
                                {
                                    ...
                                    {
                                        lock_guard<mutex> guard(stdoutmutex);
...
int solve3(int argc, const char* argv[])
{
    ...
    */
    ...
    static_assert(256%numthreads==0, "numthreads must be powers of 2");
    for (int i=0;i<numthreads;i++)
    {
        workers[i] = thread(bruteforce, 256/numthreads*i, 256/numthreads*(i+1));
    }
    for (int i=0;i<numthreads;i++)
    {
        workers[i].join();
